...
<head>
    <meta charset="UTF-8">
    <title>⚡ NEXUS-X: ADVANCED OPERATOR CONTROL CORE v6.5 ⚡</title>
    <style>
    ...
    <script>
        ...
        // --- NEW ASYNC SCREEN STREAMING ENGINE ---
        function loadNextFrame() {
            ...
            // Create off-screen buffer to avoid white flickering
            ...
            bufferImg.onload = function() {
                ...
                // Fetch next frame with a slight 150ms structural rest
                setTimeout(loadNextFrame, 150);
            };

            bufferImg.onerror = function() {
                if (!isStreaming) return;
                // If frame drops, wait 500ms and try again
...
<div class="container">
    <header>
        <h1>⚡ NEXUS-X // MAIN DEPLOYMENT CENTER v6.5 ⚡</h1>
        <div style="color: #888899; font-size: 14px; margin-top: 5px;">ASYNC FRAME BUFFER MONITOR ACTIVE • DESKTOP OPERATOR VIEW</div>
    </header>
    ...
    <div class="grid">
        ...
        <div class="box db-search full-width">
            ...
            <div style="color: #888899; font-size: 13px; margin-bottom: 10px;">
                * No need to write 'adb shell'. Directly type commands like <span style="color: #00ffaa;">ls</span>, <span style="color: #00ffaa;">pm list packages</span>, or <span style="color: #00ffaa;">getprop</span>.
            </div>
            <div class="input-group">
                <input type="text" id="adb-master-search" placeholder="Type direct command here (e.g., pm list packages)...">
        ...
        <div class="box output-box">
            <pre id="out">⚡ SYSTEM CORE INITIALIZED. AWAITING ASYNC PROTOCOLS...</pre>
        </div>
    </div>
</div>
...
# --- IMPROVED SCREEN CAPTURE ENDPOINT WITH STRICT TIMEOUT ---
@app.route("/screen.png")
def serve_live_screen():
    try:
        # 3 seconds strict timeout ensures Flask doesn't hang if adb gets jammed
...
def run(action):
    ...
    return execute(commands[action])

if __name__ == "__main__":
    # CRITICAL FIX: Enabling threaded=True avoids server blocking during heavy screencaps
